//! 笔记 / 笔记本存储层 — 后端 index.json / memo.json / .md 磁盘 IO 的总入口。
//!
//! ## v3 模块拆分 (2026/06 重构)
//!
...
//! - [`mod@index_store`]  — `index.json` / `memo.json` IO + sync 维护
...
//! 含 `.md` 后缀)。
//!
//! - id 仍由 6 位 nanoid (`MEMO_ID_ALPHABET` 字符集) 生成, 存 index.json 内部
//!   key / 深链 / noteReference 节点, **不再出现在物理文件名**。
//! - 同 title 冲突时自动追加 `-1` / `-2` / ... 后缀。
//! - index.json 始终是全量索引的真源, 任何写路径最终都过 [`mod@ops`] 的原语。
...
pub use frontmatter::{
    build_md_content, extract_body_content, extract_frontmatter_key, merge_frontmatter,
    MergeOverrides,
};
pub use index_store::MEMO_INDEX_FILENAME;
pub use ops::{
    atomic_write_bytes, base_filename, resolve_filename_conflict, sanitize_filename_component, IsMd,
};
pub use types::{
    Memo, MemoColor, MemoIndexEntry, MemoIndexFile, MemoMetadataFile, MemoTag, MemoTodoEntry,
    Notebook, NotebookConfig, ReconcileReport, TodoItem,
};
pub use versions::{
...
///   index.json / memo.json 在这里。
/// - `notebook_file_path`: 笔记本配置 (`~/.flowix/notebook.json`).
/// - `current_notebook_id`: 当前活跃 notebook id, `None` 表示走默认。
/// - `index_cache`: `<notebook>/.metadata/index.json` 的内存缓存。读路径
///   ([`MemoFile::read_index`]) 命中时直接 clone 返回, 跳过磁盘 IO + JSON parse。
///   写路径 ([`MemoFile::write_index`] / `_locked` 系列) 在落盘成功后回填。
...
impl MemoFile {
    ...
    pub fn set_current_notebook(&mut self, id: Option<String>) {
        // 切 notebook 时 `get_index_path()` 解析到的磁盘路径会变 ── 旧 cache
        // 指向的目录不再有效, 必须失效。 同 id 重复设置 (steady state) 时
    ...
    /// index.json / notebook.json 写都过 [`Self::write_index`] /
    ...
    /// `.metadata/` 目录绝对路径 — 内部 `index.json` / `memo.json` 所在地。
