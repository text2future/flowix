...
//!
//! 唯一持久化字段: `key` (= memo id, 字符集 `[0-9a-z]` 6 位)。
//!
//! ## 工具
//!
...
///
/// 用于 `register_existing_file` 走"按磁盘 key 反查 index.json entry"路径,
...
/// - `key:` value 不在 `[0-9a-z]{6}` 字符集 → None (防止用户手写垃圾值)
pub fn extract_frontmatter_key(content: &str) -> Option<String> {
    ...
    for line in inner.split('\n') {
        if let Some(c) = FM_LINE_RE.captures(line) {
            ...
            if name == "key" {
                let value = c.get(3).map(|m| m.as_str())?.trim();
                if value.len() == 6
                    && value.chars().all(|c| {
...
///
/// `key` 字符集 `[0-9a-z]` 6 位, YAML 标准下无引号合法。
