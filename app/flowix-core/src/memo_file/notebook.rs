...
//!
//! The authoritative notebook registry lives in `index.db` next to the legacy
//! `notebook.json` file. The JSON path is kept only as a migration source:
//! when the SQLite table is empty, rows are imported from that file on first
//! read.
...
impl MemoFile {
    /// Legacy notebook config path. Kept for one-way migration into `index.db`.
    pub fn get_notebook_file_path(&self) -> PathBuf {
        self.notebook_file_path.clone()
    }

    /// Global index database path. It intentionally lives next to
    /// `notebook.json` (`~/.flowix/index.db` in production) so notebook
    /// registry data stays with the rest of the user config.
    pub fn get_index_db_path(&self) -> PathBuf {
        self.notebook_file_path
            .parent()
            .map(|parent| parent.join("index.db"))
            .unwrap_or_else(|| self.app_data_path.join("index.db"))
    }
    ...
    pub(super) fn open_index_db(&self) -> std::io::Result<Connection> {
        ...
        self.migrate_legacy_notebook_json_if_needed(&conn)?;
        Ok(conn)
    }

    fn migrate_legacy_notebook_json_if_needed(&self, conn: &Connection) -> std::io::Result<()> {
        let count: i64 = conn
