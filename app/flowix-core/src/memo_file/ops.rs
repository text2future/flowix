...
impl MemoFile {
    ...
    /// 原文件名入 memo index, **不**重命名磁盘文件 (保留外部工具句柄)。
    ...
    pub fn reconcile_with_disk_bidirectional(&self) -> Result<ReconcileReport, String> {
        ...
        // 6. 派生 re-derivation 迁移: 旧 regex (停在标点) 提取的扁平 tag
        //    在升级到 path-style regex 后需要按新规则重派生。逐 memo
        //    比对新旧 tags, 不变就跳过, 变才写回 memo index。
        let notebook_id = self.current_notebook_id_for_index();
        let rederived = self
            .rederive_all_memos_for_notebook_id_locked(&notebook_id)
            .map_err(|e| format!("rederive migration failed: {e}"))?;
        if rederived > 0 {
            tracing::info!(
                "[reconcile_with_disk_bidirectional] rederived tags for {rederived} memos in notebook {notebook_id}"
            );
            list.last_updated = chrono::Utc::now().timestamp_millis();
        }
    ...
    /// 一次性迁移: 把指定 notebook 里所有 memo 的派生字段 (tags /
    /// preview / todos / agents / thumbnail) 按当前 [`apply_derived_memo_fields`]
    /// 流水线重算。仅当 `memo.tags` 与新提取结果不一致时才写回 memo
    /// index, 避免无谓 IO。
    ///
    /// **设计意图**: 旧 regex (停在标点) 把 `#中国/台湾` 截成 `["中国"]`,
    /// 升级到 path-style regex (允许 `/` 段) 后, 存量 memo 的 `memo.tags`
    /// 还是旧值; 通过此函数批量重派生, 升级到 `["中国/台湾"]`。
    /// 调用方: [`Self::reconcile_with_disk_bidirectional`] 收尾。
    pub(crate) fn rederive_all_memos_for_notebook_id_locked(
        &self,
        notebook_id: &str,
    ) -> Result<usize, String> {
        let conn = self.open_memo_index_db().map_err(|e| e.to_string())?;
        let base = self.memo_base_for_notebook_id_result(notebook_id)?;

        // 只拉 (id, filename) ── 一次性比对 memo_tags, 不动其它字段。
        let mut stmt = conn
            .prepare("SELECT id, filename FROM memos WHERE notebook_id = ?1")
        ...
        for (id, filename) in rows {
            ...
            // 只重算 tags, 其它派生字段保持不变 (preview / thumbnail /
            // todos 等已正确, 不被 regex 变更影响)。
            let new_tags = super::derivation::extract_tags_from_body(&content);
            if new_tags == old_tags {
                continue;
            }

            let now = chrono::Utc::now().timestamp_millis();
            // 写回 memo_tags: 删旧, 插新 ── 用事务保证原子性。
            ...
            // 同步 memos 表的 updated_at, 让上层判定有变更。
            tx.execute(
                "UPDATE memos SET updated_at = ?1 WHERE notebook_id = ?2 AND id = ?3",
                rusqlite::params![now, notebook_id, &id],
            )
            .map_err(|e| e.to_string())?;
