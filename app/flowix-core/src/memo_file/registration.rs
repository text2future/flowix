//! 磁盘对账 + 单文件注册 — 把外部 .md 文件纳入 memo index。
//!
//! v3: 物理文件名 = memo index entry.filename (含 `.md`), 旧版 `{title}#xxxxxx.md`
//! 约定已废。所有入口不再抽 id from filename, 也不再重命名磁盘文件。
//!
//! ## 入口
//!
//! - [`MemoFile::register_existing_file`]: 单文件注册。原文件名入 memo index,
//!   不重命名磁盘文件。
...
//!   memo index 没记录的 .md 补进来。
//! - [`MemoFile::reload_memo_from_disk`]: 重新读 .md 派生 preview / tags / todos。
//! - [`MemoFile::unregister_memo_by_path`]: 按绝对路径找 memo index entry 并移除。
//!
