use rllm::chat::{ChatMessage as LlmChatMessage, ChatRole, MessageType};
use rllm::{FunctionCall, ToolCall as LlmToolCall};

use crate::session::ChatMessage as ThreadChatMessage;
...
pub(super) fn estimate_llm_message_tokens(messages: &[LlmChatMessage]) -> u32 {
    messages
        .iter()
        .map(|m| {
...
fn persisted_to_llm(m: crate::session::ChatMessage) -> Option<LlmChatMessage> {
    match m.role.as_str() {
...
pub(super) fn build_llm_context_window(messages: Vec<ThreadChatMessage>) -> Vec<LlmChatMessage> {
