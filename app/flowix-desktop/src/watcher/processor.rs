...
/// 把 `Memo` 包成 `DispatchOutcome::Updated`, 路径用 index.json 里权威的
/// `entry.filename`, 不直接用事件 path (可能已变化)。
fn emit_updated(mf: &MemoFile, memo: Memo) -> DispatchOutcome {
    let entry_path = mf
        .get_memo_base()
        .join(&memo.filename)
        .display()
...
/// (rename 后这是新位置的绝对路径)。
fn emit_created(memo: Memo, new_abs_path: PathBuf) -> DispatchOutcome {
...
/// 抽 key → 在 index.json 里按 id 反查, 命中即用 key 对应的 entry; 不命中
/// 才退回 filename 兜底。
///
...
/// - key 不在当前 index.json: pasted/imported markdown, 新建 memo 并刷新 key
/// - 无 key + filename 在 index.json: reload (保留 id/filename, 用户保存时会注入 key)
...
fn dispatch_modify_event(memo_file: &MemoFile, path: &Path) -> Result<DispatchOutcome, String> {
    ...
    match disk_key {
        Some(id) => match memo_file.read_memo(&id) {
            ...
            Some(existing) => {
                ...
                if is_physical_rename_candidate(&old_path) {
                    rename_existing_memo(memo_file, &id, &old_path, path)
                } else {
        ...
        None => {
            ...
            if memo_file.find_memo_by_filename(&filename).is_some() {
                ...
                Ok(emit_created(memo, new_abs_path))
            }
        }
    }
}
...
fn reload_existing_memo(memo_file: &MemoFile, filename: &str) -> Result<DispatchOutcome, String> {
    ...
    Ok(emit_updated(memo_file, updated))
}
...
fn rename_existing_memo(
    ...
    let updated = memo_file.rename_memo_file(old_path, new_path)?;
    Ok(emit_updated_at(updated, new_path))
}
...
fn register_pasted_copy_as_new(
    ...
    Ok(emit_created(memo, path.to_path_buf()))
}

/// 同 [`emit_updated`] 但路径用事件原始 path (rename 场景下是新位置的绝对路径)。
fn emit_updated_at(memo: Memo, abs_path: &Path) -> DispatchOutcome {
...
impl MemoEventProcessor {
    ...
    /// - Remove:        unregister (按 filename 查 index.json, 命中删, 没命中 no-op)
    /// - Other:         忽略
    pub fn process(
        event: &RawFsEvent,
        app: &AppHandle,
    ...
    pub(crate) fn unregister_and_emit(
        ...
        // v3 物理文件名是 `<title>.md` (id 跟文件名解耦), `extract_memo_id_from_abs_path`
        // 永远返回 None, 旧实现会把空 id 发到前端, 让 `handleMemoDeleted` 的
        // `memos.filter(m => m.id !== "")` 一条都过滤不掉 -> 幽灵笔记。
        //
        // 修法: **在 `unregister_memo_by_path` 之前**按 filename 反查 index.json
        // 拿到真实 id。`unregister_memo_by_path` 内部就是用同一 filename 匹配 + 删
        // entry, 所以这里查到的 id 跟它即将删的那条是同一条, 不存在 race -- 都是
        // 走 `current_index_io` 锁串行化, 内部只读 + 写 index.json 一次。
        //
        // 拿不到 id 的两种情形:
        // - 路径里没有合法的 .md 文件名 (如 `..`): 直接放弃 emit, 反正
        //   `unregister_memo_by_path` 也会 return false, index.json 没动。
        // - filename 不在 index.json (孤立 .md / 已经被删过): 同样放弃 emit, 不凭空
        //   generate id, 保持 id 一定来自 index.json 这个不变量。
        let Some(filename) = path.file_name().and_then(|n| n.to_str()) else {
            return;
        };
        let Some(memo) = mf.find_memo_by_filename(filename) else {
            tracing::debug!(
                "[MemoWatcher] unregister_and_emit: no index.json entry for filename={}, skipping emit (unregister will also no-op)",
                filename
            );
            return;
        };
...
mod tests {
    ...
    fn fresh_memo_file() -> (MemoFile, PathBuf) {
        ...
        // 写一个最简的 notebook.json ── MemoFile::get_memo_base 只读 path
        // 字段, 其它字段缺失无所谓 (反序列化可能容忍也可能 fail, 但
        // get_memo_base 是直接读 path, 不走 NotebookConfig 解析路径)
        // NotebookConfig 字段名是 createdAt / updatedAt / isDefault (camelCase serde rename),
        // 不是 snake_case。这里跟生产配置结构对齐。
        ...
        fs::write(&notebook_file, serde_json::to_string_pretty(&cfg).unwrap()).unwrap();

        let memo_file = MemoFile::new(app_data, notebook_file);
        // 不调 set_current_notebook 的话, get_memo_base 走默认路径
        // (~/Documents/flowix) ── register_unnamed_file / write_index
    ...
    fn dispatch_modify_event_emits_created_for_unregistered_file() {
        ...
        // register_unnamed_file 会把磁盘文件重命名为带 id 后缀, new_abs_path
        // 跟原 path 不一定相同
        assert!(
            new_abs_path.exists(),
    ...
    fn dispatch_modify_event_updated_preserves_id_across_external_edit() {
        // 关键不变量: 外部改 body 后, index.json 里这条 entry 的 id 不会变
    ...
    /// 回归: 物理删除时, `unregister_and_emit` 必须能从 index.json 查到真实 id
    /// 注入到 `MemoEvent::Deleted` 里。v3 物理文件名是 `<title>.md` (id 跟
    /// 文件名解耦), 旧实现调 `extract_memo_id_from_abs_path` 永远拿到 `None`,
    /// emit `id=""` 给前端 → `memos.filter(m => m.id !== "")` 一条都过滤
    /// 不掉 → 幽灵笔记。这里直接验证修复后的核心查找逻辑:
    /// "按 filename 找 index.json entry, 拿到的 id 跟 register 时生成的 id 一致"。
    #[test]
    fn physical_delete_resolves_real_id_from_index() {
        ...
        // 修复前: id=""
        // 修复后: id 应该是 index.json 里这条 entry 的真实 id
        let memo = mf
            .find_memo_by_filename(&filename)
            .expect("seeded entry should be in index.json");
        ...
        // 必须带 index.json 的 id, 而不是从 filename 里硬猜
    ...
    /// `MemoEvent::Deleted` (id 拿不到),也不动 index.json。
    #[test]
    fn physical_delete_for_unregistered_file_is_noop() {
        ...
        // 模拟 unregister_and_emit 的 id 查找前置段: filename 不在 index.json
        let looked_up = mf.find_memo_by_filename(filename);
        assert!(
            looked_up.is_none(),
            "unregistered .md must not resolve to a index.json entry"
        );
    ...
    fn dispatch_modify_event_detects_rename_via_frontmatter_key() {
        ...
        match event {
            MemoEvent::Updated {
                ...
                assert_eq!(
                    memo.id, original_id,
                    "memo.id must match index.json entry id"
                );
                // filename 改成磁盘实际文件名
                assert_eq!(
        ...
        // 收尾: index.json 的 entry.filename 真的更新了
        let entry_after = mf
            .find_memo_by_filename(&new_filename)
            .expect("new filename should be in index.json after rename");
        assert_eq!(
            entry_after.id, original_id,
            "index.json entry's id must be preserved"
        );
        // 旧 filename 应该已经不在 index.json
        assert!(
            mf.find_memo_by_filename(&filename).is_none(),
            "old filename must be removed from index.json after rename"
        );
    ...
    fn dispatch_modify_event_rekeys_pasted_duplicate_when_original_still_exists() {
        ...
        assert_eq!(
            mf.read_memo(&original_id).unwrap().filename,
            original_filename,
    ...
    //
    // 模拟"index.json 已经被前序事件清掉, 磁盘 key 还在" ── 比如外部
    ...
    fn dispatch_modify_event_rekeys_orphan_disk_key_as_new_document() {
        ...
        // 直接造一个 .md 带 frontmatter key 但 index.json 里没记录的"孤儿"
        ...
        // 模拟 read_memo 返回 None 的状态 ── index.json 干净
        assert!(mf.read_memo(orphan_id).is_none());
        ...
        // 收尾: index.json 真的有这条 entry
        assert!(
            mf.read_memo(orphan_id).is_none(),
            "old disk key must not be registered in this notebook"
        );
        let entry = mf
            .read_memo(&memo.id)
            .expect("fresh id should now be in index.json");
