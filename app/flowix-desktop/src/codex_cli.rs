...
impl CodexCliManager {
    pub fn new(thread_manager: Arc<tokio::sync::RwLock<ThreadManager>>) -> Self {
        Self {
            ...
            sessions: CodexSessionManager::new(),
        }
    }
    ...
    async fn run_codex(
        ...
        let has_explicit_runtime_workspace = message
            .runtime_config
            .as_ref()
            .and_then(|config| config.codex.as_ref())
            .map(|config| config.cwd.is_some() || !config.workspace_paths.is_empty())
            .unwrap_or(false);
        let runtime_key = has_explicit_runtime_workspace
            .then(|| external_runtime_key(&cwd, &workspace_paths))
            .flatten();
        ...
        let previous_runtime_key = self.sessions.previous_runtime_key(thread_id).await;
        let session_id = select_codex_session_for_runtime(
            mapped_session_id,
            thread_id,
            runtime_key.as_deref(),
            previous_runtime_key.as_deref(),
        );
...
#[cfg(test)]
mod tests {
    use crate::codex::session::select_codex_session_for_runtime;
    ...
    #[test]
    fn runtime_workspace_starts_new_session_until_runtime_key_matches() {
        let mapped = Some("019f0000-0000-7000-8000-000000000000".to_string());
        assert_eq!(
            select_codex_session_for_runtime(mapped.clone(), "thread_1", Some("cwd|extra"), None),
            None
        );
        assert_eq!(
            select_codex_session_for_runtime(
                mapped.clone(),
                "thread_1",
                Some("cwd|extra"),
                Some("cwd|other"),
    ...
    #[test]
    fn real_codex_session_id_always_resumes_even_without_runtime_key_cache() {
        let session_id = "019f0000-0000-7000-8000-000000000001";
        assert_eq!(
            select_codex_session_for_runtime(None, session_id, Some("cwd|extra"), None).as_deref(),
            Some(session_id)
        );
        assert_eq!(
            select_codex_session_for_runtime(
                Some("019f0000-0000-7000-8000-000000000002".to_string()),
                session_id,
                Some("cwd|extra"),
                Some("cwd|other"),
            )
    ...
    #[test]
    fn missing_runtime_workspace_keeps_legacy_resume_behavior() {
        let mapped = Some("019f0000-0000-7000-8000-000000000000".to_string());
        assert_eq!(
            select_codex_session_for_runtime(mapped.clone(), "thread_1", None, None),
            mapped
        );
        assert_eq!(
            select_codex_session_for_runtime(
