...
pub fn run() {
    ...
    // 笔记本配置走 ~/.flowix/notebook.json, 偏好设置走 ~/.flowix/boot/preference.json。
    ...
    // 首次启动设备登记模块 ── 和上面同样的 prep 模式: clone 进 setup 闭包。
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            //   先 sleep 10s 再 POST, 与产品更新 7s 检查错开。已经登记
            //   过的 (`boot.json:registered=true`) 走 fast-path 直接跳过。
            let device_registry = Arc::new(crate::device_registration::DeviceRegistry::load(
                &user_config_dir_for_device,
            ));
            if !device_registry.is_registered() {
                device_registry.clone().spawn_first_install_registration();
            }
