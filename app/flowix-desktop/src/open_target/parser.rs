...
//! ## memo id 格式约束
//!
//! 与后端 `MemoFile::extract_memo_id_from_abs_path` 一致: 6 字符 `[0-9a-z]{6}`。
...
/// "用户想打开什么"。 resolver 层再查磁盘 / index.json 落到具体 notebook。
...
pub enum OpenTarget {
    /// 物理路径 — 走 index.json 扫所有 notebook 找匹配的 .md。
    PhysicalPath {
        path: String,
...
/// memo id 6 字符 `[0-9a-z]{6}` — 与后端 `extract_memo_id_from_abs_path` 同形。
...
fn split_scheme<'a>(raw: &'a str) -> Option<(&'a str, &'a str)> {
    ...
    //   - rest **保留**原大小写 ── memo id 在 index.json 里走 `[0-9a-z]`,
...
pub fn parse_open_target(raw: &str) -> Result<OpenTarget, OpenTargetError> {
        ...
        .or_else(|| trimmed.strip_prefix("file:///"))
    {
        ...
        // index.json filename → id 反查; parser 阶段无法给 memo_id。
...
mod tests {
    ...
    fn parses_deep_link_memo_with_id() {
        let t = parse_open_target("flowix://memo/abc123").unwrap();
        match t {
            OpenTarget::DeepLink {
                memo_id,
                notebook_hint,
                ...
                assert_eq!(memo_id.as_deref(), Some("abc123"));
    ...
    fn parses_deep_link_memo_with_notebook_hint() {
        let t = parse_open_target("flowix://memo/abc123?nb=nb_xyz").unwrap();
        match t {
            OpenTarget::DeepLink {
                memo_id,
                notebook_hint,
    ...
    fn rejects_invalid_memo_id_chars() {
        // 含大写 / `_` / `-` 都不行 (跟后端 extract_memo_id_from_abs_path 严格一致)
    ...
    fn parses_open_with_path_query() {
        ...
        // resolver 走 index.json filename → id 反查。
