...
use crate::external_run::{
    emit_chunk_with_run_id, external_runtime_key, kill_child_tree, resolve_run_id,
    select_external_session_for_runtime, ExternalRunRegistry, ExternalSessionManager,
};
use crate::hermes_history::is_hermes_session_id;
...
impl HermesCliManager {
    pub fn new(thread_manager: Arc<tokio::sync::RwLock<ThreadManager>>) -> Self {
        Self {
            ...
            sessions: ExternalSessionManager::new(),
        }
    }
    ...
    async fn run_hermes(
        ...
        let has_explicit_runtime_workspace = message
            ...
            .unwrap_or(false);
        let hermes_runtime_key = has_explicit_runtime_workspace
            .then(|| external_runtime_key(&cwd, &workspace_paths))
            .flatten();
        let previous_runtime_key = self.sessions.previous_runtime_key(thread_id).await;
        let session_id = select_external_session_for_runtime(
            mapped_session_id,
            hint,
            hermes_runtime_key.as_deref(),
        ...
        let result = async {
            ...
            self.runs
                ...
                .await;
            if let Some(runtime_key) = hermes_runtime_key.clone() {
                self.sessions
    ...
    async fn resolve_and_persist_session(
        ...
        let Ok(Some(session_id)) =
            crate::hermes_history::most_recent_session_since(started_at_ms).await
        else {
            return;
        };
        runtime_log::record_agent_event(
...
pub(crate) fn resolve_hermes_binary() -> PathBuf {
    for env_var in ["HERMES_AGENT_CLI_PATH", "HERMES_CLI_PATH"] {
    ...
    #[cfg(windows)]
    {
        ...
        PathBuf::from("hermes.cmd")
    }

    #[cfg(not(windows))]
    {
        if let Ok(found) = which_hermes() {
            return found;
        }
        if let Some(home) = dirs::home_dir() {
            let candidates = [
...
#[cfg(not(windows))]
fn which_hermes() -> Result<PathBuf, ()> {
