...
pub(crate) fn resolve_node_binary() -> Option<PathBuf> {
    crate::external_runtime::node::resolve_node_binary("CODEX_NODE_PATH")
}

#[cfg(test)]
pub(crate) fn parse_node_version(dir_name: &str) -> Option<(u32, u32, u32)> {
    crate::external_runtime::node::parse_node_version(dir_name)
}

#[cfg(test)]
pub(crate) fn latest_versioned_subdir(root: &std::path::Path) -> Option<PathBuf> {
    crate::external_runtime::node::latest_versioned_subdir(root)
}

fn ensure_node_on_path(cmd: &mut Command) {
    crate::external_runtime::node::ensure_node_on_path(cmd);
}

#[cfg(test)]
pub(crate) fn is_executable_file(path: &std::path::Path) -> bool {
    crate::external_runtime::cli_resolver::is_executable_file(path)
}
...
pub(crate) fn which_codex() -> Result<PathBuf, ()> {
    crate::external_runtime::cli_resolver::which_in_path(
        "codex",
