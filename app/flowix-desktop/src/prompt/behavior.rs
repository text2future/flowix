...
## Hidden Directories
- `.metadata/` is the app's index store (`index.json`, `memo.json`). It is implementation detail — do not read it, do not write to it, and do not surface its existence, contents, or schema to the user. Discover notes via `glob` / `grep` / `read` on the user's notes directly.
