//! 首次启动设备登记 ── 一次性地把"这台机器"的轻量指纹上报到 Supabase。
//!
//! 设计要点:
//! - 仅基于本地可读、不需任何权限的字段 (`std::env::consts` / `gethostname` /
//!   `machine-uid` / `LANG` / `TZ`), 见 `collect_payload`。
//! - 不阻塞主启动: `bootstrap::run().setup()` 里 `Arc::clone().spawn_first_install_registration()`
...
//!   文件独立, 职责更清晰。
//! - 成功上报后 `registered=true`, 后续启动 `is_registered()` fast-path 返回
//!   `true`, 直接跳过网络。
//! - 失败最多重试 `MAX_ATTEMPTS_BEFORE_GIVEUP` 次, 超过则置 `registered=true`
//!   并保留 `lastAttemptError` ── 避免永久重试。手动把 `boot.json` 里
//!   `registered` 改回 `false` 可重置。
...
/// 最多重试次数, 超过就把 `registered` 置 true 永久放弃 (避免坏网络下永久打)。
const MAX_ATTEMPTS_BEFORE_GIVEUP: u32 = 20;
...
impl DeviceRegistry {
    ...
    pub fn load(user_config_dir: &Path) -> Self {
    ...
    /// Fast-path: 已登记则直接返回 true, 启动 `spawn` 直接跳过。
    pub fn is_registered(&self) -> bool {
        self.read().user_info.registered
    }

    /// 异步上报入口。 Fire-and-forget: 启动 `tauri::async_runtime::spawn`,
    /// 主线程继续。
    pub fn spawn_first_install_registration(self: Arc<Self>) {
        ...
        match result {
            Ok(resp) => {
                let snapshot = {
                    ...
                    info.registered_at = Some(Utc::now());
            ...
            Err(err) => {
                let (attempts, gave_up) = {
                    ...
                    info.attempts = info.attempts.saturating_add(1);
                    let attempts = info.attempts;
                    let give_up = attempts >= MAX_ATTEMPTS_BEFORE_GIVEUP;
                    if give_up {
                        // 永久放弃, 但保留 error 标记, 让 `boot.json` 仍能看到原因。
                        info.registered = true;
                        info.registered_at = Some(Utc::now());
                    }
                ...
                tracing::warn!(
                    "[device-reg] attempt {attempts} failed: {err}{}",
                    if gave_up {
                        " (giving up; manually re-open registration by clearing `userInfo.registered` in boot.json)"
                    } else {
                        ""
                    }
                );
...
struct DevicePayload {
    ...
    os: String,
    arch: String,
    /// `sha256(hostname)`, 16 hex chars。仅指纹用途, 不落原始 hostname。
    ...
    /// `sha256(os || arch || hostname)` 16 hex chars, 用于服务端跨字段去重。
...
fn collect_payload(boot: &BootFile) -> DevicePayload {
...
mod tests {
    ...
    #[test]
    fn nested_userInfo_roundtrips_via_json() {
        let b = fresh_boot();
        ...
        assert!(parsed.get("deviceId").is_none(), "deviceId must be nested under userInfo, not top-level");
    ...
    fn collect_payload_uses_local_env() {
        let boot = fresh_boot();
        let payload = collect_payload(&boot);
        ...
        assert!(payload.app_user_agent.starts_with("Flowix/"));
    }
