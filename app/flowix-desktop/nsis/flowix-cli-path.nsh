...
!define FLOWIX_CLI_SHIM "${FLOWIX_CLI_BIN_DIR}\flowix-cli.cmd"
!define FLOWIX_ALIAS_SHIM "${FLOWIX_CLI_BIN_DIR}\flowix.cmd"
...
!macro NSIS_HOOK_POSTINSTALL
  ...
  ${If} $0 != ""
    ...
    FileClose $0
  ${EndIf}
  ; `flowix` 别名 ── 短名, 跟 `flowix-cli` 同内容。 不冲突因为桌面
  ; `flowix.exe` 在 $INSTDIR (默认不在 PATH), wrapper 在 $LOCALAPPDATA\Flowix\bin
  ; (NSIS 加入 PATH 的目录)。 只有用户手动把 $INSTDIR 也加进 PATH 时
  ; 才会有遮蔽 ── 那种情况是用户选择, 不归我们管。
  FileOpen $0 "${FLOWIX_ALIAS_SHIM}" w
  ${If} $0 != ""
    FileWrite $0 "@echo off$\r$\n"
    FileWrite $0 "$\"$INSTDIR\flowix-cli.exe$\" %*$\r$\n"
    FileClose $0
