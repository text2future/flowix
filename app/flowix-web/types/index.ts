...
export const SUPPORTED_TEXT_EXTENSIONS = [
  ...
  '.html', '.css', '.scss', '.xml', '.yaml', '.yml', '.log',
  '.sh', '.bash', '.zsh', '.gitignore', '.env', '.conf', '.ini'
] as const;
...
export interface FlowixRPCSchema {
  bun: {
    requests: {
      // Chat
      "chat:send": {
        params: { message: string; threadId: string; headers?: Record<string, string> };
      ...
      "files:importToNotebook": {
        params: { files: { name: string; content: string }[]; spacePath?: string };
