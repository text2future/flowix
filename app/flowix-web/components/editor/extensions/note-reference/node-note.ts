...
import { openNoteByMemoId, openNoteByPhysicalPath, resolveMemoById, resolveMemoByPath } from './memo-resolver';
import { escapeHtml, parseBooleanAttr, pickAttr, splitDisplay, stripMdSuffix, unescapeHtml } from './markdown';
...
const STRICT_FLOWIX_MEMO_HREF_RE = /^flowix:\/\/memo\/([0-9a-z]{6})(?:\?[^)\s]*)?$/;
const VALID_MEMO_ID_RE = /^[0-9a-z]{6}$/;
...
class NoteReferenceView implements ProseMirrorNodeView {
  ...
  private async handleClick(e: MouseEvent): Promise<void> {
    ...
    // 优先用 memoId 反查 (flowix://memo/<id> 深链), 后端走 index.json 扫
