...
import { LazyDocumentEditor } from './lazy-document-editor';
...
export function DocumentContainer({
  ...
  const {
    ...
    setState,
    reloadDocument,
  } = useDocumentContent({ identity: documentIdentity, memoId, isExternalDocument, transitionId });
  ...
  if (state.error) {
    // 物理文件丢失场景: index.json 还有这条 entry, 但磁盘上 .md 没了。
    ...
    // (ops.rs:411-414) 只清 index.json, 然后 emit MemoEvent::Deleted。
