...
// 物理路径是否需要 finalize (旧 P1 修复遗留: 写盘后 index.json `path`
// 字段临时态 vs 物理文件 rename 之间的 race)。v3 后:
// - 物理 rename 不再发生 (filename 由 index.json 持有, 后端 rename_memo
//   一次同步物理文件 + index.json, 没有 race 窗口)
// - index.json `path` 字段已删, memo 物理路径 = notebookPath + memo.filename
