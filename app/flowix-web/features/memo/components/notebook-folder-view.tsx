...
export function NotebookFolderView({
  ...
  const noteTree = useMemo(() => ({
    ...tree,
    rootChildren: sortNotebookTreeItems(tree.rootChildren.filter(isNotebookTreeItemVisible), sort),
    nodes: new Map([...tree.nodes].map(([path, item]) => [
      path,
      item.children
        ? { ...item, children: sortNotebookTreeItems(item.children.filter(isNotebookTreeItemVisible), sort) }
        : item,
    ])),
  }), [sort, tree]);
