...
//   setActiveAgentTypeKey 守门), 它们与 chat-store-migration 内部的
//   副本彼此独立, 因为 chat-store-migration 内部依赖相同 lib, 不暴露。
...
/**
 * Legacy localStorage 适配层 (activeThreadId → activeThreadIds / ...) ──
 * 完全下放到 chat-store-migration.ts。 chat-store 只持有运行时 ChatStore
 * 形状, 不再关心 localStorage 兼容性细节。
 */
