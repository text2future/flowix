...
export function buildInitialInstanceRuntimeConfig(): RuntimeConfig {
  ...
  const workspace = seedWorkspace || notebookWorkspace || undefined;
  // 冻结种子里的 folders/notebooks + 全局 enabled ── 只要有一处存在都算上,
  // dedupe 用 Set, 剔除空串。 用户上次勾的目录 (即便全局 toggle off 了) 也保留。
  const folders = Array.from(
    new Set(
      [...seedFolders, ...foldersFromAccess].filter((path) => path && path.length),
    ),
  );
  const notebooks = Array.from(
    new Set(
      [...seedNotebooks, ...notebooksFromAccess].filter(
