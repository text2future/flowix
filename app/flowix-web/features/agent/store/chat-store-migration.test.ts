...
describe("chat-store-migration / mergeChatPersisted", () => {
  ...
  it("legacy activeThreadId 字段被折算到 activeThreadIds.flowix", () => {
    const merged = mergeChatPersisted(
      { activeThreadId: "legacy-thread-id" },
      emptyCurrent,
    );
    expect(merged.activeThreadIds?.flowix).toBe("legacy-thread-id");
  });

  it("legacy 当前类型字段优先于 legacy 单值字段", () => {
    const merged = mergeChatPersisted(
      {
        activeThreadIds: { codex: "new-codex-id" },
        activeCodexThreadId: "legacy-codex-id",
      },
      emptyCurrent,
    );
    expect(merged.activeThreadIds?.codex).toBe("new-codex-id");
  });

  it("legacy threadList 折算到 threadLists.flowix", () => {
    const merged = mergeChatPersisted(
      {
        threadList: [
    ...
    expect(merged.threadLists?.flowix).toEqual([
      { threadId: "t1", title: "T1", createdAt: 1, updatedAt: 1 },
    ]);
    expect(merged.threadLists?.codex).toEqual([]);
  });
