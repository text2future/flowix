...
import { formatToolName, truncateStart, extractFileName } from "../../../../lib/message/format";
...
function ToolBody({ toolName, result }: { toolName?: string; result: ToolResult }) {
  ...
  return <CompactCode value={valueToText(data ?? result)} />;
}
...
export function MessageTool({ message }: { message: ChatMessage }) {
  ...
  const summary = inputSummary(message.toolInput);
