...
fn binary_prints_version_and_help() {
    ...
    assert!(stdout(&version).starts_with("flowix-cli "));
...
fn binary_reports_usage_errors_with_expected_exit_code() {
    ...
    assert!(stderr(&missing).contains("usage: flowix-cli list <notebook>"));
