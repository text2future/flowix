...
> 记录 MCP（Model Context Protocol）的协议定位、角色、消息模型、生命周期、传输层、Server/Client 能力，端到端工具调用流程，以及落地实现建议。
...
所有实现都必须支持 base protocol 和 lifecycle；其他能力，例如 tools、resources、prompts、sampling、roots，是按需声明和协商的能力。
