...
- **dev**：`npm run tauri:dev` → `app/flowix-desktop/tauri.conf.dev.json` → bundle ID `com.flowix.app` / `Flowix Dev`
...
dev 与生产当前共用 bundle ID 是为了让 macOS TCC / security-scoped bookmark 尽量识别为同一个应用身份，减少用户选择文件夹后仍反复弹权限确认。视觉上仍通过 bundle name / 窗口标题区分（`Flowix Dev` vs `Flowix`）。如果未来需要 dev / prod 完全并存隔离，再恢复 dev 专用 bundle ID，但会牺牲本地文件夹授权的复用稳定性。
