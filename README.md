...
Flowix ships with a standalone `flowix` command that **shares the same `memo_file` storage** as the desktop app. Edits from the terminal are visible in the desktop UI within ~1 second (and vice versa) via the filesystem watcher.
